...
host_renderer_presentation matrix=29 vertex=14 texture=11 triangles=10 geom_mode=21 tex_images=2 tex_malformed=2 tex_segmented=2 tex_resolved=0 tex_unresolved=2 color_images=20 depth_images=9 tile_setup=226 texture_loads=38 combine=14 sync=23 fill_rect=10 othermode=38 packets=58
...
host_renderer_backend packets=465 geometry=53 state=84 texture=266 target=39 sync=23 address_refs=74 valid_refs=25 invalid_refs=49
...
It does **not** boot the game yet. The next blocker is branch-target guarding plus real memp/resource provenance before RT64/custom command execution: the first bounded generated task now scans `2236` commands across `4` display lists, resolves all three segmented branch-display-list addresses, summarizes RSP/RDP work (`1716`/`520`), records `58` presentation packet candidates, emits named packet previews, validates `25` of `74` backend address-bearing refs, separates `2` real-display-list texture-image candidates and `2` malformed display-list image commands from `12` payload false positives, and delivers the scheduler done message; a two-task guarded probe (`GOLDENEYE_RSP_TASK_LIMIT=2`) advances through two scheduler-done cycles with the same texture-classification shape. The runtime still needs branch-target guarding and texture/resource candidates to resolve to backed asset/memory addresses before F3DEX/RDP command mapping into RT64 or a custom renderer can produce meaningful output, plus stricter replacement of scheduler/video/audio/input placeholders.
