...
| Current blocker | Guarded `mainproc` now clears `guPerspectiveF`, advances through host-simulated frame ticks, consumes generated RSP/display-list tasks, recursively walks the first bounded display-list task, validates backend resource addresses, emits backend packet previews, and delivers scheduler done messages back to the game loop. |
...
The game does **not** boot yet. The next milestone is turning validated backend packet previews into executable renderer commands: translate F3DEX/RDP packets into RT64 or a custom presentation backend, then fix the remaining texture-image provenance and runtime placeholders behind the generated game loop.
...
- The host renderer shim now recursively walks generated display-list tasks (`host_renderer_execute`) with bounded command/list/depth limits, classifies RSP/RDP presentation commands, prints a top-opcode histogram, validates backend resource addresses, previews texture image candidates with command addresses/raw words/segment state, and emits backend packet records (`host_renderer_backend_packet[...]`) with opcode names, raw words, resolved addresses, and validity flags. The latest guarded first task scans `2236` commands across `4` display lists, reaches depth `1`, resolves all `3` segmented branch-list references, sees `1716` RSP-style and `520` RDP-style commands, summarizes `29` matrix, `14` vertex, `10` triangle, `10` valid-format texture-image candidates plus `6` malformed `0xFD` candidates, `226` tile-setup, `38` texture-load, `14` combine, and `58` presentation packet commands, and validates `25` of `82` backend address-bearing refs. It queues `OS_SC_DONE_MSG` back into `gfxFrameMsgQ` and stops after the bounded done message. A two-task guarded probe (`GOLDENEYE_RSP_TASK_LIMIT=2`) advances through two scheduler-done cycles with the same renderer summary shape. The current blocker is real asset decompression / memory-pool bank modeling so texture-image commands become backed by usable resource addresses before RT64/custom rendering, scheduler, video, audio, input, and controller behavior are completed.
