...
| Current blocker | Guarded `mainproc` now reaches bounded RSP/display-list tasks; the branch guard skips payload/data branch targets by default, validates top-level backend resource addresses, emits packet previews, and delivers scheduler done messages back to the game loop. |
...
The game does **not** boot yet. The next milestone is real memp/resource provenance, then translating only proven F3DEX/RDP packets into RT64 or a custom presentation backend.
